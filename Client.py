...
class TftpProcessor(object):
    ...
    def FileRead(self):
        ...
        while count <= len(my_File):
            ...
            count = count + 512
        count = 0
        print("here" + str(len(arrOfPackets)))  # valid
        while count < len(arrOfPackets):
            my_String = arrOfPackets.pop(count)
            my_String_in_ASCII = bytes(my_String, "ascii")
            my_packet = struct.pack(
                str(len(my_String_in_ASCII)) + "s", my_String_in_ASCII
            )
            print(my_packet)  # valid
    ...
    if command_type == "push":
        ...
        while tftpObj.has_pending_packets_to_be_sent():
            ...
            if response == PackNSent:
                ...
                my_Socket.sendto(PacketTBSent, my_Server_adr)
